...
export function authConfiguration() {
  ...
  const googleAllowedDomain = process.env.GOOGLE_ALLOWED_DOMAIN?.trim().toLowerCase().replace(/^@/, '') || '';
  return { email, passwordHash, jwtSecret, googleClientId, googleAllowedDomain };
}
...
export function createToken(email, secret, claims = {}) {
...
export function isAuthorizedSession(payload, configuration) {
  if (!payload || !configuration || !payload.email) return false;
  if (payload.provider === 'google') {
    return Boolean(configuration.googleClientId && configuration.googleAllowedDomain && payload.hd === configuration.googleAllowedDomain);
  }
